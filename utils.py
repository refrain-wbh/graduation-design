...
def read_config(config_path = "config/default.yaml"):
    default_config = edict(yaml.load(open("config/default.yaml", 'r', encoding='utf-8')))
...
def read_configs(config_path):
    ...
    config_loader = [edict(update_dict(copy.deepcopy(default_config),edict(config))) for config in config_loader]
...
class HuggingfaceDataset(Dataset):
    def __init__(
        ...
        self.task_name = self.config.task_name if hasattr(self.config, 'task_name') else None 
    ...
    def random_select(self):
        example_num = self.config.num_train_examples_ratio * len(self.dataset) // self.config.num_labels
        unselect_label_nums = {label: example_num for label in range(self.args.num_labels)}
        selected_indexs = []
    ...
    def _format_examples(self, examples):
        key1,key2 = task_to_keys[self.name if self.task_name is None else self.task_name]
        texts = ((examples[key1],) if key2 is None else (examples[key1], examples[key2]))
        inputs = self.tokenizer(*texts, truncation=True, max_length=self.config.max_seq_length, return_tensors='pt')
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            return [self._format_examples(self.dataset[j]) for j in range(idx.start, idx.stop)]
