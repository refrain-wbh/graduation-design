    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        Parameters:
            ...
            force_download: (`optional`) boolean, default False:
                Force to (re-)download the model weights and configuration files and override the caches if they exists.
        ...
        if pretrained_model_name_or_path is not None:
            if os.path.isdir(pretrained_model_name_or_path):
                ...
                archive_file = hf_bucket_url(
                    ...
                    revision=revision, # modified 之前是使用cdn的
                    mirror=mirror,
                )
...
def prune_linear_layer(layer, index, dim=0):
    ...
    W = layer.weight.index_select(dim, index).clone().detach()# 选中对应的weight
    ...
    new_layer = nn.Linear(new_size[1], new_size[0], bias=layer.bias is not None).to(layer.weight.device)# 新的liner layer，在
    new_layer.weight.requires_grad = False
    new_layer.weight.copy_(W.contiguous())# 复制过来原来的weight
