...
def robust_statistics(
    model, train_dev_loader_with_idx, adv_setting, use_cur_preds=True
):
    config = adv_setting
    statistics = {
        i.as_py(): {} for i in train_dev_loader_with_idx.dataset.get_column("idx")
    }
    ...
    for model_inputs, labels, idxs in pbar:
        ...
        for cur_logits, cur_label, cur_pred, idx in zip(logits, labels, preds, idxs):
            cur_losses = F.cross_entropy(
                cur_logits.unsqueeze(0), cur_label.unsqueeze(0)
            )
            ...
            single_statistics = {
                ...
                "original_probability": nn.Softmax(dim=-1)(cur_logits)[
                    cur_label.item()
                ].item(),
            }
    ...
    for model_inputs, labels, idxs in pbar:
        ...
        if config.adv_init_mag > 0:
            ...
            if config.adv_norm_type == "l2":
                ...
                delta = torch.zeros_like(embedding_init).uniform_(
                    -config.adv_init_mag, config.adv_init_mag
                ) * input_mask.unsqueeze(2)
        else:
        ...
        for astep in range(config.adv_steps):
            ...
            # loss.backward()
            settings.get_accelerator().backward(loss)
            ...
            if astep == config.adv_steps - 1:
                for (
                    ...
                    idx,
                    cur_delta,
                ) in zip(logits, labels, preds, idxs, delta):
                    cur_losses = F.cross_entropy(
                        cur_logits.unsqueeze(0), cur_label.unsqueeze(0)
                    )
                    cur_loss = torch.mean(cur_losses)
                    single_statistics = {
                        ...
                        "after_perturb_probability": nn.Softmax(dim=-1)(cur_logits)[
                            cur_label.item()
                        ].item(),
            ...
            if config.adv_norm_type == "l2":
                ...
                delta = (
                    delta + config.adv_lr * delta_grad / denorm
                ).detach()  # detach 截断反向传播?
                if config.adv_max_norm > 0:
                    ...
                    reweights = (
                        config.adv_max_norm / delta_norm * exceed_mask
                        + (1 - exceed_mask)
                    ).view(-1, 1, 1)
...
def robust_statistics_fgsm(
    model, train_dev_loader_with_idx, adv_setting, use_cur_preds=True
):
    # rename
    config = adv_setting
    statistics = {
        idx: {} for idx in train_dev_loader_with_idx.dataset.get_column("idx")
    }
    ...
    with torch.no_grad():
        ...
        for model_inputs, labels, idxs in pbar:
            logits = model(**model_inputs).logits
            preds = logits.argmax(dim=-1)
            for cur_logits, cur_label, cur_pred, idx in zip(
                logits, labels, preds, idxs
            ):
                cur_losses = F.cross_entropy(
                    cur_logits.unsqueeze(0), cur_label.unsqueeze(0)
                )
    ...
    for model_inputs, labels, idxs in pbar:
        ...
        if config.adv_norm_type == "l2":
            ...
            if config.adv_max_norm > 0:
                ...
                exceed_mask = (delta_norm > config.adv_max_norm).to(embedding_init)
                reweights = (
                    config.adv_max_norm / delta_norm * exceed_mask + (1 - exceed_mask)
                ).view(-1, 1, 1)
                delta = (delta * reweights).detach()
        elif config.adv_norm_type == "linf":
            denorm = torch.norm(
        ...
        logits = model(**batch).logits
        preds = logits.argmax(dim=-1)
        for cur_logits, cur_label, cur_pred, cur_delta, idx in zip(
            logits, labels, preds, delta, idxs
        ):
            cur_losses = F.cross_entropy(
                cur_logits.unsqueeze(0), cur_label.unsqueeze(0)
            )
...
def prepare_datamap(config):
    ...
    for epoch in range(train_setting.epochs):
        logger.info("Epoch: {}/{}".format(epoch, train_setting.epochs))
        avg_loss = utils.ExponentialMovingAverage()
        model.train()
        pbar = tqdm(train_loader)
        for model_inputs, labels, _ in pbar:
            if global_step % int(statistics_setting.interval * epoch_steps) == 0 and (
                statistics_setting.with_untrained_model or global_step != 0
            ):
                logger.info("statistics_interval...")
                statistics_func = (
                    robust_statistics_fgsm
                    if statistics_setting.use_fgsm
                    else robust_statistics
                )
                cur_robust_statistics = statistics_func(
                    model,
                    train_loader,
                    adverisal_setting,
                    statistics_setting.use_cur_preds,
                )
        ...
        if train_setting.do_eval:
            ...
            if accuracy > best_accuracy:
                logger.info(
                    f"Best performance so far.accracy: {accuracy}, epoch: {epoch}"
                )
    ...
    # 保存统计量，但是config另外存储
    npy_time = time.strftime(
        "%Y-%m-%d-%H:%M:%S", time.localtime(time.time() + 8 * 60 * 60)
    )
    npy_file = f"statistics/npys/{npy_time}.npy"
    np.save(npy_file, robust_statistics_dict)
...
if __name__ == "__main__":
    yaml_file = "statistics/data_map_with_fsm.yaml"
